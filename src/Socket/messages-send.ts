...
export const makeMessagesSocket = (config: SocketConfig) => {
	...
	const Ditss = makeNewsletterSocket(config)
	const {
		ev,
		authState,
		processingMutex,
	...
	const getUSyncDevices = async(jids: string[], useCache: boolean, ignoreZeroDevices: boolean) => {
		...
		const result = await Ditss.executeUSyncQuery(query)
	...
	const relayMessage = async(
		...
		msgId = msgId || generateMessageIDV2(Ditss.user?.id)
			...
			if(
				...
				const fullMsg = await generateWAMessage(
					...
					content,
					{
						...
						getProfilePicUrl: Ditss.profilePictureUrl,
