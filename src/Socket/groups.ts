...
export const makeGroupsSocket = (config: SocketConfig) => {
	const Ditss = makeChatsSocket(config)
	const { authState, ev, query, upsertMessage } = Ditss
	...
	const groupFetchAllParticipating = async() => {
		...
		Ditss.ev.emit('groups.update', Object.values(data))

		return data
	}

	Ditss.ws.on('CB:ib,,dirty', async(node: BinaryNode) => {
		...
		await Ditss.cleanDirtyBits('groups')
	})
			...
			await upsertMessage(
				{
					key: {
						...
						id: generateMessageIDV2(Ditss.user?.id),
						fromMe: false,
