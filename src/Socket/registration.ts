...
export const makeRegistrationSocket = (config: SocketConfig) => {
	const Ditss = makeBusinessSocket(config)
	...
	const register = async(code: string) => {
		...
		const result = await mobileRegister({ ...Ditss.authState.creds, ...Ditss.authState.creds.registration as RegistrationOptions, code }, config.options)

		Ditss.authState.creds.me = {
			id: jidEncode(result.login!, 's.whatsapp.net'),
			name: '~'
		}

		Ditss.authState.creds.registered = true
		Ditss.ev.emit('creds.update', Ditss.authState.creds)
	...
	const requestRegistrationCode = async(registrationOptions?: RegistrationOptions) => {
		...
		Ditss.authState.creds.registration = registrationOptions
