...
const startDitss = async() => {
	...
	const reply = async (jid: string, msg: AnyMessageContent, options: object) => {
		...
		await Ditss.sendMessage(jid, msg, options);
	};
				...
				switch (command) {
					...
					default:
						if (customPrefix.test(body)) {
							...
							try {
								// @ts-ignore
								let exec = new (async () => { }).constructor('print', 'm', 'Ditss', 'chat', 'process', 'args', 'require', _text);
								_return = await exec.call(Ditss, (...args) => {
									if (--i < 1) return;
									return reply(chat, { text: format(...args) }, { quoted: m });
								}, m, Ditss, chat, process, args, require);
							} catch (e) {
								_return = e;
							} finally {
								await Ditss.sendMessage(chat, { text: format(_return) }, { quoted: m });
							};
						};
				}
			};
